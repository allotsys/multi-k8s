...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=allotsys/multi-server:$SHA
kubectl set image deployments/client-deployment client=allotsys/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=allotsys/multi-worker:$SHA